...
const TouristExplore = () => {
  ...
  // (chưa dùng) const EXIT_BUFFER_FACTOR = 0.85;
  ...
  const prepareAIVoice = async (text) => {
    try {
      const res = await api.get("/pois/ai/tts", {
        params: { text, language: currentLang },
      });
  ...
  useEffect(() => {
    ...
    if (closestPoi) {
      ...
      if (prevPoiId !== closestPoi.id) {
        // stop TTS hiện tại và cắt sang vùng mới
        ...
        const transitionText = `Đang chuyển thuyết minh sang ${closestPoi.name}`;
        toast.success(transitionText, { icon: "🔊" });

        (async () => {
          const noticeAudio = await prepareAIVoice(transitionText);
          if (noticeAudio) {
            activeAudioRef.current = noticeAudio;
            await noticeAudio.play().catch(() => {});
          }

          const narrationText = closestPoi.description || "";
          if (!narrationText.trim()) return;

          const narrationAudio = await prepareAIVoice(narrationText);
          if (narrationAudio) {
            activeAudioRef.current = narrationAudio;
    ...
    // ra khỏi tất cả vùng
    if (!closestPoi && activePoiIdRef.current) {
      const prevPoiId = activePoiIdRef.current;
      const prevPoi = pois.find((p) => p.id === prevPoiId);
      activePoiIdRef.current = null;

      const leaveText = prevPoi?.name
        ? `Bạn đã ra khỏi vùng ${prevPoi.name}`
        : "Bạn đã ra khỏi vùng";

      toast.success(leaveText, { icon: "📍" });
      ...
      (async () => {
        ...
        const leaveAudio = await prepareAIVoice(leaveText);
        if (leaveAudio) {
          activeAudioRef.current = leaveAudio;
          await leaveAudio.play().catch(() => {});
        }
      })();
    }
  }, [userLoc, pois, currentLang]);
    ...
    <div className="fixed inset-0 md:ml-64 ml-0 pt-16 bg-white z-10 overflow-hidden flex flex-col">
      ...
      {nearbyPois.length > 0 && (
        <div
          ...
          <div
            className={`max-w-7xl mx-auto px-4 transition-opacity duration-300 ${isExpanded ? "opacity-100" : "opacity-0 pointer-events-none"}`}>
                ...
                <div
                  ...
                  <div className="absolute top-1 right-1 z-10 mt-1 opacity-80 group-hover/item:opacity-100 transition-opacity">
                    <Button
                      ...
                      onClick={(e) => {
                        e.stopPropagation();
                        setNearbyPois((prev) =>
                          prev.filter((item) => item.id !== poi.id),
                        );
                      }}>
                  ...
                  <div className="flex flex-col gap-1">
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      <span className="text-[10px] text-gray-400">(TTS)</span>
                    </div>
                  </div>
                </div>
              ))}
        ...
        <div className="flex flex-col items-center gap-6">
          ...
          <div className="flex flex-col items-center w-full">
            ...
            <p className="mt-4 text-gray-500 text-sm">
              Vui lòng đưa camera vào mã QR tại cửa hàng
            </p>
          </div>
        </div>
      </Modal>
            ...
            <div className="flex flex-col h-[90vh] overflow-y-auto border-r border-gray-100 scrollbar-thin scrollbar-thumb-gray-200">
              ...
              <div className="p-8">
                <div className="flex items-start justify-between gap-4 mb-4">
                  ...
                  {getTourIdFromPoi(selectedPoi) && (
                    <Button
                      variant="outline"
                      size="sm"
                      className="!py-2"
                      onClick={() => {
                        const tourId = getTourIdFromPoi(selectedPoi);
                        setIsModalOpen(false);
                        navigate(`/tourist/tours?tourId=${tourId}`);
                      }}>
                      Xem chi tiết Tour
                    </Button>
                  )}
            ...
            <div className="flex flex-col h-full bg-gray-50/50">
              ...
              <div className="flex-1 p-6 overflow-y-auto flex flex-col gap-4">
                <div className="bg-white p-4 rounded-2xl rounded-tl-none shadow-sm border border-gray-100 max-w-[85%]">
                  <p className="text-sm text-gray-800">
                    Xin chào! Mình là Laura, trợ lý ảo của{" "}
                    <b>{selectedPoi.name}</b>. Bạn có thông tin nào muốn biết về
                    quán, cứ hỏi mình nhé!
                  </p>
                </div>
              ...
              <div className="p-4 bg-white border-t border-gray-100">
                ...
                <p className="mt-2 text-[10px] text-gray-400 text-center">
                  AI có thể trả lời sai, hãy xác thực lại thông tin với chủ
                  quán.
                </p>
              </div>
            </div>
