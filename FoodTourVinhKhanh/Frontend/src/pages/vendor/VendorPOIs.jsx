...
const VendorPOIs = () => {
      ...
      try {
        ...
        if (res.data.success) {
          ...
          setFormData({
            ...
            knowledge:
              poi.knowledge && poi.knowledge.length > 0
                ? poi.knowledge
                : [{ category: "menu", content: "" }],
            thumbnail: poi.thumbnail,
            banner: poi.banner,
          });
          if (poi.localized_data && poi.localized_data.length > 0) {
            setLocalizedData(
              poi.localized_data.filter((d) => d.lang_code !== "vi"),
            );
          } else {
            setLocalizedData([]);
          }
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.localized.name.trim())
      return toast.error("Vui lòng điền tên địa điểm!");
    if (!formData.localized.description.trim())
      return toast.error("Vui lòng viết mô tả");
    ...
    try {
      ...
      toast.error(
        typeof serverError === "string"
          ? serverError
          : serverError?.message || "Lỗi hệ thống!",
      );
    } finally {
      setLoading(false);
    }
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div>
              <h1 className="!text-4xl font-black text-gray-900 uppercase">
                Quản lý <span className="text-blue-600">Địa điểm</span> Của bạn
                ({quota.current_total}/{quota.max_pois})
              </h1>
              <p className="mt-2 text-sm text-gray-500">
                Gói hiện tại còn có thể tạo thêm {quota.remaining} POI.
              </p>
            </div>
            <div className="space-x-5">
              <Button
                onClick={() => setIsMapShowing(true)}
                className="shadow-lg">
                Bản đồ
              </Button>
              <Button
              ...
              <div
                ...
                <div className="relative h-44 overflow-hidden">
                  ...
                  <div className="absolute top-4 right-4">
                    <span
                      className={`px-3 py-1.5 rounded-full text-[10px] font-black uppercase tracking-wider shadow-sm ${
                        poi.is_Active
            ...
            <form onSubmit={handleSubmit} className="space-y-4 mt-2">
              ...
              <div className="space-y-2">
                <label className="text-sm font-bold text-gray-700 ml-1 flex items-center gap-2">
                  <MapPin size={16} className="text-red-500" /> Xác định vị trí
                  trên bản đồ
                </label>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  ...
                  <Button
                    type="button"
                    variant="outline"
                    className="h-full bg-white border-blue-100 text-blue-600"
                    size="sm"
